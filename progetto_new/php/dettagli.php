...
if(isset($_GET["id_casa"])){
      ...
      if(isset($_SESSION['username'])){
            ...
            $richieste = $connect->db_getArray('SELECT * FROM richieste WHERE username LIKE "'.$_SESSION['username'].'" AND id_casa LIKE "'.$_GET["id_casa"].'"');
            if(isset($richieste))
            if(isset($preferito)){
            ...
            if(isset($_SESSION['isAdmin'])){
                  $opzioni = '<div id="specsAnnounce" class="containerSpecsFull">';
                  $opzioni .= '<hr><h2>Opzioni Admin</h2><div id="opzioni_admin">';
                  $opzioni .= '<a class="buttonSearch" href="rimuovi_casa.php?id_casa='. $_GET["id_casa"].'">Rimuovi</a> ';
                  $opzioni .= '<a class="buttonSearch" href="modifica_casa.php?id_casa='. $_GET["id_casa"].'">Modifica</a>';
                  $opzioni .= '</div></div>';

                  $output = str_replace('<div id="specsAnnounce" class="containerSpecsFull"></div>',$opzioni,$output);
            }
            else{
                  $output = str_replace('<div id="specsAnnounce" class="containerSpecsFull">',"",$output);
            }
      }
}
else{
